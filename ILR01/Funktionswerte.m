...
f_s = 5e3; % gewünschte Abtastrate [Hz]
A = 0.1; % Amplitude des Ausgangssignals [V]
...
filename = './ILR01/Funktionswerte.h';

% Perioden ausrechnen
T = 1/f;
T_s = 1/f_s;

t = linspace(0, T,  T/T_s);

% um halbe Amplitde verschieben
y = (2^bit_DAC-1) / V_ref * (A*sin(2*pi*f*t)+A*2);
...
fprintf(fID, 'const unsigned int table[]PROGMEM = {\n');
for i=1:(length(y)-1)
